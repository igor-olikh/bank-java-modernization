...
public class DataSeeder {
    ...
    private void seedCustomers() {
        ...
        Account jamesCheck = openAcc(james, AccountType.CHECKING, "USD", "15432.50", 0);
        Account jamesSave  = openAcc(james, AccountType.SAVINGS,  "USD", "48200.00", 1);
        ...
        Account mariaCheck = openAcc(maria, AccountType.CHECKING, "EUR", "8750.00", 0);
        ...
        Account yukiSave   = openAcc(yuki, AccountType.SAVINGS,     "JPY", "1250000.00", 0);
        Account yukiInvest = openAcc(yuki, AccountType.INVESTMENT,  "JPY", "3800000.00", 1);
        ...
        Account mohammedBiz = openAcc(mohammed, AccountType.BUSINESS, "SAR", "320000.00", 0);
        ...
        Account amaraCheck = openAcc(amara, AccountType.CHECKING, "GHS", "4800.00", 0);
        ...
        Account lenaSave = openAcc(lena, AccountType.SAVINGS, "EUR", "22500.00", 0);
        ...
        Account priyaCheck = openAcc(priya, AccountType.CHECKING, "INR", "185000.00", 0);
        ...
        Account lucasCheck = openAcc(lucas, AccountType.CHECKING, "BRL", "9200.00",  0);
        Account lucasSave  = openAcc(lucas, AccountType.SAVINGS,  "BRL", "31500.00", 1);
        ...
        Account irinaSave = openAcc(irina, AccountType.SAVINGS, "RUB", "980000.00", 0);
        ...
        Account chenInvest = openAcc(chen, AccountType.INVESTMENT, "CNY", "850000.00",  0);
        Account chenBiz    = openAcc(chen, AccountType.BUSINESS,   "CNY", "1200000.00", 1);
        ...
        Account fatimaCheck = openAcc(fatima, AccountType.CHECKING, "MAD", "12400.00", 0);
        ...
        Account erikCheck = openAcc(erik, AccountType.CHECKING, "SEK", "85000.00", 0);
        ...
        Account aikoSave = openAcc(aiko, AccountType.SAVINGS, "JPY", "680000.00", 0);
        ...
        Account samuelBiz   = openAcc(samuel, AccountType.BUSINESS,  "NGN", "5800000.00", 0);
        Account samuelCheck = openAcc(samuel, AccountType.CHECKING,  "NGN", "1200000.00", 1);
        ...
        Account sofiaCheck = openAcc(sofia, AccountType.CHECKING, "EUR", "6300.00", 0);
        ...
        Account noamCheck = openAcc(noam, AccountType.CHECKING, "ILS", "42000.00",  0);
        Account noamSave  = openAcc(noam, AccountType.SAVINGS,  "ILS", "115000.00", 1);
        ...
        Account mikhailCheck = openAcc(mikhail, AccountType.CHECKING, "ILS", "28500.00", 0);
        ...
        Account igorCheck = openAcc(igor, AccountType.CHECKING, "ILS", "55000.00",  0);
        Account igorSave  = openAcc(igor, AccountType.SAVINGS,  "ILS", "180000.00", 1);
    ...
    /**
     * Opens an account and back-dates it relative to the customer's join date.
     * accountIndex 0 = same day as customer joined;
     * accountIndex 1+ = random offset (60..730 days) after the previous.
     */
    private Account openAcc(Customer customer, AccountType type, String currency,
                            String initialBalance, int accountIndex) {
        Account account = accountService.openAccount(customer.getCustomerId(), type, currency);
        // Compute the opening date: first account on join day, each extra one later
        LocalDateTime base = customer.getCreatedAt();
        if (accountIndex > 0) {
            // add 60..730 days per extra account so later accounts are plausibly newer
            int extraDays = 60 + RNG.nextInt(671); // 60..730
            base = base.plusDays((long) accountIndex * extraDays);
            // Cap at today so we never go into the future
            LocalDateTime today = LocalDateTime.now();
            if (base.isAfter(today)) base = today.minusDays(RNG.nextInt(30));
        }
        account.setOpenedAt(base);
